...
describe("ModelBrowser row label", () => {
	...
	test("renders all six audited ground-truth models with clean name, separate provider, and no raw ids", () => {
		const fixtures = [
			{ provider: "Antigravity", id: "antigravity/gemini-3.8-flash", name: "Gemini 3.8 Flash" },
			{ provider: "Antigravity", id: "antigravity/claude-opus-4-6-thinking", name: "Claude Opus 4.6 Thinking" },
			{ provider: "Codex", id: "codex/gpt-5.6-sol", name: "GPT-5.6 Sol" },
			{ provider: "OpenCode Zen", id: "opencode-zen/big-pickle", name: "Big Pickle" },
			{ provider: "OpenCode GO", id: "opencode-go/deepseek-v4-pro", name: "DeepSeek V4 Pro" },
			{ provider: "Cerebras", id: "cerebras/gemma-4-31b", name: "Gemma 4 31B" },
		];

		for (const fixture of fixtures) {
			const browser = new ModelBrowser(Settings.isolated({}));
			browser.setItems(buildBrowserItems([makeNamedModel(fixture.provider, fixture.id, fixture.name)]));

			const row = Bun.stripANSI(browser.render(140)[2]);
			expect(row).toContain(fixture.name);
			expect(row).toContain(fixture.provider);
			expect(row).not.toContain(fixture.id);
			expect(row).not.toContain(`${fixture.provider}/${fixture.name}`);
			expect(row).not.toContain(`${fixture.provider}/${fixture.id}`);
		}
	});

	test("preserves selector, wire id, and technical search across all facets", () => {
		const fixture = { provider: "OpenCode GO", id: "opencode-go/deepseek-v4-pro", name: "DeepSeek V4 Pro" };
		const model = makeNamedModel(fixture.provider, fixture.id, fixture.name);
		const other = makeNamedModel("Other", "other/dummy-model", "Other Model");
		const expectedSelector = `${fixture.provider}/${fixture.id}`;

		const searchQueries = [fixture.name, fixture.id, fixture.provider, expectedSelector];

		for (const query of searchQueries) {
			const browser = makeBrowser([other, model], []);
			browser.setQuery(query);
			const selected = browser.getSelected();
			expect(selected).toBeDefined();
			expect(selected?.selector).toBe(expectedSelector);
			expect(selected?.model.provider).toBe(fixture.provider);
			expect(selected?.model.id).toBe(fixture.id);
			expect(selected?.model.name).toBe(fixture.name);
		}
	});
